...
import { getAttachmentTypeFromMime } from './utils';
import { reducer, initialState } from './state';

const apiMaxNumberOfFiles = 10;

export const useMessageInput = props => {
    const { clearEditingState, message, noFiles, parent, publishTypingEvent = true } = props;
    ...
    const handleChange = useCallback(
        async newValue => {
            dispatch({
                text: editorRef.current.serializeMessage(newValue),
                type: 'setValue',
                value: newValue,
            });

            if (publishTypingEvent && newValue && channel) {
    ...
    return {
        ...
        isUploadEnabled,
        queueUploads,
        totalAttachments: (fileOrder?.length || 0) + (imageOrder?.length || 0),
    };
};
